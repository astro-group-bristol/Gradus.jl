...
function solve_equitorial_circular_orbit(
    ...
    __solve_equitorial_circular_orbit(
        m, r, optimizer, lower_bound, upper_bound
    )
end

function sliding_window(
    func, N, lower_bound, upper_bound,
...
function solve_equitorial_circular_orbit(
    ...
    lower_rate = 0.98, upper_rate = 1.5,
    kwargs...
) where {T}
    ...
    candidate_vϕ = sliding_window(
        ...
        r = r_range_reverse[i]
        solve_equitorial_circular_orbit(m, r, lower_bound = lower_bound, upper_bound = upper_bound)
    end
    reverse!(candidate_vϕ)
end

function trace_equitorial_circular_orbit(
    m::AbstractMetricParams{T}, rs; kwargs...
) where {T}
    map(zip(
            rs,
            solve_equitorial_circular_orbit(m, rs; kwargs...)
        )
    ) do (r,vϕ)
