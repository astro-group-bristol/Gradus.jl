...
function _find_offset_for_radius(
    ...
    zero_atol = 1e-7,
    worst_accuracy = 1e-3,
    initial_r = max(10 * one(r_target), r_target),
    accel = 0.9,
    kwargs...,
)
    ...
    point, df, y = step(x)
    i = 0
    while (!isapprox(y, 0, atol = zero_atol))
        # do the Newton update
        x = x - accel * y / df

        # calculate next step
        point, df, next = step(x)
        if (i > 1) && isapprox(log2(abs(y)), log2(abs(next)), rtol = worst_accuracy / 100)
            break
        end
        y = next
