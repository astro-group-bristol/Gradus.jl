...
@testset "cunningham-transfer-functions" begin
    ...
    for (angle, expected) in zip(
        ...
        ctf =
            cunningham_transfer_function(m, u, d, 4.0, 2000.0; finite_diff_order = 5, num_points = 200)
        s = sum(ctf.f)
    ...
    for (r, expected) in zip(
        ...
        ctf = cunningham_transfer_function(m, u, d, r, 2000.0; finite_diff_order = 5, num_points = 200)
        s = sum(ctf.f)
        @test isapprox(s, expected, atol = 1e-2, rtol = 0.0)
    end
end
